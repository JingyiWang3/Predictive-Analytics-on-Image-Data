...
```{r model_setup}
shrinks_range <- c(0.01,0.1,0.3) #for gbm
trees_range  <- c(40,50,60) #for gbm
...
if(run.cv){
err_cv <- cv(rgb_feature ,label_train,
             run.xgboost = F,
             run.gbm = F,
             run.adaboost = F,
             K = 5, 
...
if(run.tune){
  ...
  param <- tune(dat_train ,label_train,
                 run.xgboost = F,
                 run.gbm = F,
                 run.adaboost = F)$best_par
  }
